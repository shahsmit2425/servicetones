...
Application/server settings live in **Render Environment Groups**:

- `servicetones-development`
- `servicetones-stagging`
- `servicetones-production`

Each group's values are shared by that environment's web service and mail worker. Enter the same **names** with different **values** in each group. Do not commit values to Git or make branch-specific config files. A local `.env` is supported for development and is ignored by Git.

`render.yaml` supplies non-secret defaults and separately connects both services to their own Postgres database. Add the remaining values manually to the group. Render does not support `sync: false` inside environment groups.
...
Variables: `SITE_URL`, `RENDER_SERVICE_ID`, `RENDER_WORKER_SERVICE_ID`, `MOBILE_RELEASES_ENABLED`.
...
See [DEPLOYMENT.md](DEPLOYMENT.md) for each signing secret's format. Mobile packaging fetches the public config from Render, avoiding a second set of Firebase web values in GitHub.
